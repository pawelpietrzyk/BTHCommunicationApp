...
--*******************************************************************/
int HTTPConnection::run(int argc, wchar_t * argv[])
{
    ULONG           retCode;
...
//
	...
	this->postOnStatusChange(HTTPConnection::STATE_LISTENING);
	this->DoReceiveRequests(hReqQueue);
...
--*******************************************************************/
DWORD HTTPConnection::DoReceiveRequests(
    IN HANDLE hReqQueue
    )
{
    ULONG              result;
    ...
    if(NO_ERROR == result)
        {
			...
			this->postOnStatusChange(HTTPConnection::STATE_CONNECTED);
...
DWORD HTTPConnection::SendHttpResponse(
    IN HANDLE        hReqQueue,
...
DWORD HTTPConnection::SendHttpPostResponse(
    IN HANDLE        hReqQueue,
    IN PHTTP_REQUEST pRequest
    )
{
